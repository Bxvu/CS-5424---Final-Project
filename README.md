...
We also noticed several usability issues. Some users found it difficult to use pupil tracking and head control together, and as a result ended up relying only on pupil control. However, pupil control alone was less sensitive for controlling the panel board, which made precise interaction difficult. Additionally, many people felt that keeping a fixed position on the screen was challenging, since head and eye movements can occur slightly even when users are unconscious of them.

We learned a lot from this feedback and are currently working on a plan to improve the usability of the robot arm. We also aim to make the arm more interactive and capable of performing a wider range of tasks.
