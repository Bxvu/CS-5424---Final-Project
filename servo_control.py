...
SERVO_MIN = 0
SERVO_MAX = 180
SERVO_CH = 0  # Default channel, will be updated from gesture control
SERVO_CH2 = 1  # Channel 0 by default
SERVO_CH3 = 0  # Channel 0 by default
SHARED_FILE = "active_channel.json"  # File to read active channel from gesture control

servo = pi_servo_hat.PiServoHat()
servo.restart()
global angle, active_servo_channel
angle = 0
active_servo_channel = SERVO_CH  # Start with default channel
print(f"Sweeping servo on channel {SERVO_CH} from {SERVO_MIN} to {SERVO_MAX} degrees...")

def read_active_channel():
    """
    Read the active servo channel from the shared file written by gesture_servo.py
    Returns the channel number, or the default if file doesn't exist.
    """
    global active_servo_channel
    try:
        if os.path.exists(SHARED_FILE):
            with open(SHARED_FILE, 'r') as f:
                data = json.load(f)
                new_channel = data.get('channel', SERVO_CH)
                if new_channel != active_servo_channel:
                    active_servo_channel = new_channel
                    print(f"\n>>> Switched to {data.get('name', 'unknown')} (Channel {new_channel}) <<<")
                return active_servo_channel
    except Exception as e:
        pass  # Silently ignore errors, keep current channel
    return active_servo_channel
...
def servo_worker():
    ...
    current_angle = angle
    while True:
        # Check for channel updates from gesture control
        current_channel = read_active_channel()
        ...
        if abs(diff) > 0.5:
            ...
            servo.move_servo_position(current_channel, current_angle)
